...
class HarnessConfig:
    """Configuration for launching the local Freddie SDK runtime."""
...
class HarnessClient:
    """Synchronous JSON-RPC client for the Freddie SDK runtime over stdio."""
    ...
    def close(self) -> None:
        ...
        try:
            ...
            proc.wait()
        self._proc = None
        self._fail_waiters(self._runtime_closed_error("Freddie runtime closed"))
            ...
            while True:
                ...
                if deadline is not None:
                    ...
                    if remaining <= 0:
                        ...
                        raise TimeoutError(
                            f"{method} timed out waiting for Freddie runtime{suffix}"
                        )
    ...
    def _write_message(self, message: JsonObject) -> None:
        ...
        if proc is None or proc.stdin is None:
            raise TransportClosedError("Freddie runtime is not running")
        try:
            ...
            raise self._runtime_closed_error("Failed to write to Freddie runtime") from exc
    ...
    def _reader_loop(self) -> None:
        ...
        try:
            ...
            self._fail_waiters(self._runtime_closed_error("Freddie runtime stdout closed"))
    ...
    def _default_launch_args(self) -> tuple[str, ...]:
        ...
        try:
            ...
            raise FileNotFoundError(
                "Unable to locate the bundled Freddie SDK runtime. "
                "Install freddie-runtime-bin or set HarnessConfig.runtime_bin."
            ) from exc
