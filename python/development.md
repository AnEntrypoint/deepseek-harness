...
`python/sdk/tests/test_bundled_runtime.py` exercises available bundled carriers and skips a carrier when its artifact has not been built. The JavaScript workspace has no automated test suite ([why](../.agents/notes/implemented/architecture/2026-09-02-buildless-workspace-no-transformation-at-launch.md)); this Python SDK's own `pytest` suite is unaffected and stays.
...
- Set `FREDDIE_RUNTIME_MODE=node` to use the built Node carrier on system Node `>=22.19`. The build script refreshes this carrier, but distributions never include or auto-select it.
...
The root `package.json` version is authoritative for both Python distributions. The staging script injects that version into both wheels and pins the SDK to the same `freddie-runtime-bin` version.
...
pip install \
  "dist-python/freddie_sdk-$version-py3-none-any.whl" \
  "dist-python/freddie_runtime_bin-$version-py3-none-macosx_14_0_arm64.whl"
```
