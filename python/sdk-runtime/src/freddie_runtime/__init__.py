"""Locate the bundled Freddie SDK runtime shipped with this package.
...
- **node (dev-only)**: the full deploy closure under ``runtime/node/``
  ...
  runtime/node/node_modules/@freddie/freddie-sdk-jsonrpc-demo/lib/packaged-bin.js`` on a
...
PACKAGE_METADATA_FILENAME = "freddie-runtime.json"
...
_EXE_ACQUISITION_HINT = (
    ...
    "freddie checkout, or install the matching `freddie-runtime-bin` platform "
...
def bundled_package_dir() -> Path:
    ...
    if not metadata.is_file():
        raise FileNotFoundError(f"freddie-runtime-bin is missing {metadata}")
    return root
...
def bundled_default_config_path() -> Path:
    ...
    if not path.is_file():
        raise FileNotFoundError(
            f"freddie-runtime-bin is missing the default runtime config at {path}"
        )
    return path
...
def bundled_runtime_path() -> Path:
    ...
    if not path.is_file():
        raise FileNotFoundError(
            f"freddie-runtime-bin is missing the runtime executable at {path}. "
            + _EXE_ACQUISITION_HINT
        )
    ripgrep = Path(f"{path}-rg")
    if not ripgrep.is_file():
        raise FileNotFoundError(
            f"freddie-runtime-bin is missing the ripgrep sidecar at {ripgrep}. "
            + _EXE_ACQUISITION_HINT
        )
    if tag.startswith("macos-"):
        ...
        if not helper.is_file():
            raise FileNotFoundError(
                f"freddie-runtime-bin is missing the node-pty spawn helper at {helper}. "
                + _EXE_ACQUISITION_HINT
            )
    return path
...
def resolve_bundled_launch_args(mode: str | None = None) -> tuple[str, ...]:
    ...
    raise ValueError(
        f"unsupported Freddie runtime mode {selected!r}: expected 'exe' or 'node' "
...
def _node_launch_args() -> tuple[str, str]:
    ...
    if not bin_js.is_file():
        raise FileNotFoundError(
            ...
            f"(no {bin_js}); run `scripts/build-exe-for-python-sdk.ts` in a freddie "
