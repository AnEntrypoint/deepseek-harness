 ...
 */
export const testInvariantCompanions =
  import.meta.glob('../packages/*/*/src/invariant.ts')
...
RegistryService.prototype.plugin = function(plugin, config, getOuterStack) {
 ...
 */
export function usesManualInvariantTree(testPath) {
...
const ALL_COMPANION_TESTS = ['/scripts/test-invariants.spec.ts']
 ...
 */
export function testInvariantCompanionPaths(testPath) {
  const normalized = testPath.replaceAll('\\', '/')
...
function startInvariantHost(root) {
  const byCallback = new Map()
  const barrierOwners = new WeakSet()
  const mount = (plugin, config) => {
...
function hasBarrierOwner(host, ctx) {
  let fiber = ctx.fiber
  while (true) {
    if (
...
async function requireActive(fiber, label) {
  await fiber.await()
...
function withInvariantReadiness(plugin, callback) {
  return {
    apply: callback,
    inject: {
...
function joinInvariantStartup(
  ...
  const joined = Object.create(fiber)
