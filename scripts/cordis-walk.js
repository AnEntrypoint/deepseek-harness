...
const MERGE_HEAD = /declare module ['"](?:@freddie\/cordis|\.\/context\.ts)['"]/
 ...
 * BLOCK — a file may legally hold several `declare module '@freddie/cordis'` blocks
...
export function contextMergeFiles(
  ...
  const out = []
  const rels = [...new Set(globSync(patterns, { cwd: scanRoot }).map(s => s.split(sep).join('/')))].sort()
...
/** Every cordis module-merge body in `sf`: `declare module '@freddie/cordis'` (harness
...
function cordisModuleBodies(sf) {
  const bodies = []
  for (const stmt of sf.statements) {
    if (!ts.isModuleDeclaration(stmt) || !ts.isStringLiteral(stmt.name)) continue
    if (stmt.name.text !== '@freddie/cordis' && stmt.name.text !== './context.ts') continue
...
export function cordisModuleBody(sf) {
  return cordisModuleBodies(sf)[0] ?? null
}

/**
 * Every `key: Type` property a `declare module '@freddie/cordis'` Context merge
 ...
 */
export function contextKeyMap(body, sf) {
...
/**
 * Every event name a `declare module '@freddie/cordis'` Events merge declares in one
 ...
 */
export function eventNameList(body, sf) {
  const names = []
  for (const stmt of body.statements) {
