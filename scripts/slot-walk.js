...
const SLOTS_MODULE = '@freddie/freddie-client-ui-slots'

/** Cheap textual prefilter for a slot-contract merge, quote-style agnostic. */
const MERGE_HEAD = /declare module ['"]@freddie\/dsh-client-ui-slots['"]/
 ...
 */
export function scanSlotFiles(scanRoot, patterns) {
  const out = []
  const names = new Map()
  const rels = [...new Set(globSync(patterns, { cwd: scanRoot })
 ...
 */
export function indexExportedTypes(scanRoot, patterns) {
  const index = new Map()
 ...
 */
export function slotDeclarations(file) {
  const out = []
 ...
 */
export function slotRegistrations(file) {
  const out = []
  const visit = (node) => {
 ...
 */
export function standardKitMembers(files, interfaceName) {
  const out = []
 ...
 */
function isSlotsReceiver(receiver, sf) {
...
function packageNameOf(scanRoot, rel, cache) {
...
function declarationText(statement, sf) {
...
function literalMember(entry, name) {
...
function namedMember(entry, name) {
...
function componentText(argument, sf) {
