 ...
 */
export function discoverPluginDirs(root = repoRoot) {
  const dirs = []
 ...
 */
export function discoverLibraryDirs(root = repoRoot) {
  const dirs = []
...
export async function watchClientPlugins(
  ...
  const readiness = { initializedBuilds: 0 }
 ...
 */
function spawnStage(stage, command, args, local) {
...
if (isMain) {
  ...
  const stop = () => { for (const stage of stages) stage.kill() }
