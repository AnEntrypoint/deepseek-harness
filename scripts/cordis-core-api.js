...
import { checkParams, checkReturns, parseJsDoc, parseTags, pointer, rawJsDoc, reportViolations } from './jsdoc.js'
import { cordisModuleBody } from './cordis-walk.js'

const root = resolve(import.meta.dirname, '..')
const FENCE = 'ts cordis-catalog'

/** Explicit editorial grouping for the pinned Cordis core API. */
export const CORDIS_CORE_API_PAGES = [
  {
...
function sourceJsDoc(text, sf, node) {
...
function headingParams(parameters, sf) {
...
function isPublicInstance(member) {
...
function isPublicStatic(member) {
  const modifiers = ts.getCombinedModifierFlags(member)
...
function memberDoc(ctx, where, name, group, rel) {
  ...
  const functionMembers = group.filter((member) =>
  ...
  for (const clause of statement.heritageClauses ?? []) {
    for (const type of clause.types) {
      ...
      const cls = sf.statements.find(
        (entry) => ts.isClassDeclaration(entry) && entry.name?.text === targetName,
      )
...
function contextMergeMembers(ctx, rel) {
...
function classMembers(ctx, rel, className) {
  const { sf, text } = load(ctx, rel)
  const cls = sf.statements.find(
    (statement) =>
...
function declarationPaste(ctx, rel, symbol) {
  const { sf, text } = load(ctx, rel)
...
function renderMember(prefix, member) {
...
export function renderCordisCoreApiPage(
  ...
  const ctx = { scanRoot, cache: new Map(), violations: [] }
  const lines = [
...
export function renderCordisCoreApiPages(scanRoot = root) {
