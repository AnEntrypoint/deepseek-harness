...
import { repositoryConfigHost } from './ts-project.js'
...
export class RepositoryCleaner {
  ...
  assertRepositoryTarget(path) {
    this.assertDescendant(this.root, path, path)
  }

  assertDescendant(root, path, displayPath) {
  ...
  async addIfPresent(targets, path, canonicalRoot) {
