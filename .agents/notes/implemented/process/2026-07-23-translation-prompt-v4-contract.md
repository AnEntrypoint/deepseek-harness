...
The executable contract lived in the renderer, request assembler, parser, and response consumer (`scripts/translation-prompt.ts`, removed along with the rest of the bilingual documentation system). Unit tests covered both directions, request order, placeholder validation, target-path validation, strict section order and cardinality, fenced responses, inline tag mentions, delimiter lines inside Markdown bodies, and frontmatter-preserving new-pair switcher correction. A keyless subprocess snapshot pinned the assembled prompt and five reviewed example turns together with a frontmatter-bearing recorded response consumed through the target-path correction.
