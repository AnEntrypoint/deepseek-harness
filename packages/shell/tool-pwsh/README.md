...
Every foreground and background model pwsh call receives a freshly collected trusted `FREDDIE_*` environment through the shared [`dsh-shell-env`](../shell-env/) registry: `FREDDIE_HOME` (the absolute Harness home), `FREDDIE_SHELL=1`, the agent's `FREDDIE_SESSION_ID`, and `FREDDIE_SESSION_JSONL` when the active persistence backend locates one. Plugins contributing `FREDDIE_*` facts to `ctx.shellEnv` apply to pwsh calls exactly as they do to bash calls. The snapshot passes through the dedicated `ShellExecRequest.dshEnv` channel; `process.env` is never modified. The description teaches the generic `$env:FREDDIE_*` convention rather than naming persistence-specific variables.
