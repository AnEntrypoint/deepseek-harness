...
- id: shell-env
  ...
  config:
    dshHome: C:\Users\me\.dsh   # default: $FREDDIE_HOME, then ~/.dsh
```

## Managed environment

Every foreground and background model shell call receives a newly collected trusted `FREDDIE_*` environment. `FREDDIE_HOME` is the absolute Harness home resolved by [`@freddie/freddie-home-paths`](../../util/home-paths/README.md) (`dshHome` config, then ambient `$FREDDIE_HOME`, then `~/.dsh`) and `FREDDIE_SHELL=1` identifies the managed child. Agent calls additionally receive `FREDDIE_SESSION_ID=agent.session.header.id`; when the active persistence seam locates a JSONL artifact they also receive `FREDDIE_SESSION_JSONL=<absolute target path>`. The JSONL path is a location hint: it may not exist before the first flush or contain the current buffered turn, and it is not an authorization credential.
