...
export class DshCordisRunRow extends HTMLElement {
  #props: CordisRunRowProps | null = null
  #observed: string | null = null

  setProps(props: CordisRunRowProps): void {
    this.#props = props
    this.#render()
  }
    ...
    const vdom = (
      ...
      >
        <div class={css.row ?? ''}>
          ...
          <span class={css.title ?? ''}>{t(card.mode === 'update' ? 'row.updateTitle' : 'row.runTitle')}</span>
        ...
        {reading === 'removed' && <div class={css.message ?? ''}>{t('run.removed')}</div>}
        {reading === 'superseded' && <div class={css.message ?? ''}>{t('run.superseded')}</div>}
        {reading === 'failed' && attempt?.error !== undefined && (
          <div class={css.message ?? ''}>{attempt.error.message}</div>
        )}
        {showBusiness && card.pluginId !== null && card.packageId !== null && card.pluginRunId !== null && (
          <div class={css.business ?? ''} data-cordis-business-view={key}>
            {renderSlot('tool.view.cordis', {
              ...
              entryKey: key,
              fallback: card.output === null ? null : <pre class={css.output ?? ''}>{card.output}</pre>,
            }) as unknown as VNode}
          </div>
        )}
        {!showBusiness && reading !== 'removed' && reading !== 'superseded' && card.output !== null && (
          <pre class={css.output ?? ''}>{card.output}</pre>
        )}
      </div>
    ) as unknown as VNode
    applyDiff(this, vdom)
...
if (typeof customElements !== 'undefined' && customElements.get('dsh-cordis-run-row') === undefined) {
  customElements.define('dsh-cordis-run-row', DshCordisRunRow)
}
