 ...
 * lightbox.
 */
export class DshDropOverlay extends HTMLElement {
  #props: DropOverlayProps = { disabled: false, labels: { title: '' } }

  setProps(props: DropOverlayProps): void {
    this.#props = props
    this.#render()
  }
  ...
  #render(): void {
    ...
    const vdom = (
      <div class={css.mask ?? ''} role="status">
        <div class={css.wrap ?? ''}>
          <div class={css.illustration ?? ''} aria-hidden="true">
            {disabled ? <UploadDisabledIllustration /> : <UploadIllustration />}
          </div>
          <div class={css.title ?? ''}>{labels.title}</div>
          {!disabled && labels.desc !== undefined && <div class={css.desc ?? ''}>{labels.desc}</div>}
        </div>
      </div>
    )
    applyDiff(this, vdom)
...
if (typeof customElements !== 'undefined' && customElements.get('dsh-drop-overlay') === undefined) {
  customElements.define('dsh-drop-overlay', DshDropOverlay)
}
 ...
 * @returns the mounted `dsh-drop-overlay` element; keep it and pass it back in to update, `.remove()` when done with it. */
export function renderDropOverlay(el: DshDropOverlay | null, props: DropOverlayProps): DshDropOverlay {
  const target = el ?? (() => {
    const created = document.createElement('dsh-drop-overlay') as DshDropOverlay
...
export function DropOverlay(props: DropOverlayProps): JSX.Element {
  return renderDropOverlay(null, props) as unknown as JSX.Element
}

/** Tilted photo-and-note cards (DeepSeek Chat upload illustration). */
function UploadIllustration(): VNode {
  return (
    <svg width="115" height="84" viewBox="0 0 115 84" fill="none" xmlns="http://www.w3.org/2000/svg">
      <g clip-path="url(#dshDropOverlayClip)">
        ...
        <path d="M77.5088 26.3047L101.057 33.7966" stroke="white" stroke-width="3" />
        <path d="M72.2646 42.7871L86.3938 47.2823" stroke="white" stroke-width="3" />
        <path d="M74.8867 34.5469L98.4353 42.0388" stroke="white" stroke-width="3" />
        <rect x="31.583" y="38.6641" width="44.9157" height="44.3666" rx="12" transform="rotate(-0.134233 31.583 38.6641)" fill="#3964FE" />
        <path d="M38.9521 73.0337C39.6129 71.7086 41.7113 66.0937 43.5113 61.1663C44.1607 59.3885 46.7484 59.3923 47.4591 61.1465C48.9728 64.8828 50.7969 68.6922 51.9988 69.1925C54.2946 70.1482 57.9854 59.3573 68.0064 70.1801" stroke="white" stroke-width="3" />
...
function UploadDisabledIllustration(): VNode {
  return (
    <svg width="115" height="84" viewBox="0 0 115 84" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <path d="M77.5088 26.3047L101.057 33.7967" stroke="white" stroke-width="3" />
      <path d="M72.2646 42.7871L86.3938 47.2823" stroke="white" stroke-width="3" />
      <path d="M74.8867 34.5469L98.4353 42.0388" stroke="white" stroke-width="3" />
      ...
      <path d="M54 70.7969C61.732 70.7969 68 64.5289 68 56.7969C68 49.0649 61.732 42.7969 54 42.7969C46.268 42.7969 40 49.0649 40 56.7969C40 64.5289 46.268 70.7969 54 70.7969Z" stroke="white" stroke-width="3.5" />
      <path d="M44 46.7969L64 66.7969" stroke="white" stroke-width="3.5" stroke-linecap="round" />
    </svg>
  )
}
