...
export class DshCustomProviderCard extends HTMLElement {
  ...
  #render() {
    ...
    const vdom = h('div', { class: styles['editor'] ?? '' },
      h('div', { class: styles['editorHeader'] ?? '' },
        h('span', { class: styles['editorTitle'] ?? '' }, t('customTitle')),
      ),
      h('div', { class: styles['field'] ?? '' },
        h('span', { class: styles['fieldLabel'] ?? '' }, t('customRoute')),
        h('input', {
          class: styles['input'] ?? '',
          type: 'text',
          value: this.#route,
          ...
          onchange: (event) => { this.#route = event.target.value; this.#render() },
        }),
      ),
      ...
      routeInvalid || routeTaken
        ? h('p', { class: styles['error'] ?? '' }, t(routeInvalid ? 'customRouteInvalid' : 'customRouteTaken'))
        : h('p', { class: styles['advancedHint'] ?? '' }, t('customRouteHint')),
      h('div', { class: styles['field'] ?? '' },
        ...
        h('input', {
          ...
          disabled: profileDisabled,
          onchange: (event) => { this.#displayName = event.target.value; this.#render() },
        }),
      ),
      h('div', { class: styles['field'] ?? '' },
        ...
        h('input', {
          ...
          disabled: profileDisabled,
          onchange: (event) => { this.#baseURL = event.target.value; this.#render() },
        }),
      ),
      h('div', { class: styles['field'] ?? '' },
        ...
        h('select', {
          ...
          disabled: profileDisabled,
          onchange: (event) => { this.#protocol = event.target.value; this.#render() },
        },
        props.protocols.map(choice => h('option', { key: choice, value: choice }, choice)),
        ),
      ),
      h('div', { class: styles['field'] ?? '' },
        ...
        keyFailure === undefined
          ? null
          : h('p', { class: styles['error'] ?? '' }, t(keyFailure === 'keyBlank' ? 'keyBlankNew' : keyFailure)),
      ),
      h(ModelListEditor, {
        models: this.#models,
      ...
      hint === undefined ? null : h('p', { class: styles['advancedHint'] ?? '' }, hint),
      h(EditorFooter, {
        t,
        busy: this.#busy,
 ...
 */
export function renderCustomProviderCard(el, props) {
  const target = el ?? document.createElement('dsh-custom-provider-card')
...
export function CustomProviderCard(props) {
  return renderCustomProviderCard(null, props)
}
