...
export class DshConversationRoot extends HTMLElement {
  #props: ConversationRootProps | null = null
  #pickerOpen = false
  #pendingWorkspaceId: WorkspaceId | undefined
  #pickerAnchor: { current: HTMLButtonElement | null } = { current: null }
  #seatObserver: ResizeObserver | null = null
  #seatEl: HTMLDivElement | null = null
  ...
  setProps(props: ConversationRootProps): void {
    this.#props = props
  ...
  #bindSeatObserver(): void {
    const seat = this.querySelector<HTMLDivElement>('[data-composer-seat]')
  ...
  #render(): void {
    ...
    const heroWorkspaceRow = (
      <div class={css.heroWorkspaceRow ?? ''}>
        ...
        {renderSlot('conversation.hero.workspace', {
          ...
          onPick: (workspaceId: WorkspaceId) => {
    ...
    const composerSeat = (
      <div class={css.composerSeat ?? ''} data-composer-seat="">
        {composer}
      </div>
    )

    const vdom = (
      <div class={css.root ?? ''} data-phase={phase}>
        {renderSlot('conversation.session.header', {})}
        <div class={css.scrollBody ?? ''} data-conversation-scroll="">
...
if (typeof customElements !== 'undefined' && customElements.get('dsh-conversation-root') === undefined) {
  customElements.define('dsh-conversation-root', DshConversationRoot)
}

/** One-shot creation/update helper preserving the original function-component call shape. */
export function ConversationRoot(props: ConversationRootProps): JSX.Element {
  const el = document.createElement('dsh-conversation-root') as DshConversationRoot
  el.setProps(props)
