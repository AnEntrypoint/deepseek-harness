 ...
 * @module @freddie/freddie-typert-generator/analyzer
 */
...
class FaceAnalyzer {
    ...
    for (const sourceFile of reachable) {
      for (const statement of sourceFile.statements) {
        if (!ts.isModuleDeclaration(statement)
          ...
          || statement.name.text !== '@freddie/cordis'
      ...
      for (const statement of sourceFile.statements) {
        ...
        if (statement.exportClause !== undefined && ts.isNamespaceExport(statement.exportClause)) {
          const namespace = this.resolveSymbol(
            this.checker.getSymbolAtLocation(statement.exportClause.name),
          )
  ...
  gatewayFieldBinding(declaration) {
    const candidates = declaration.members.filter((member) =>
  ...
  typeMetaMapMembers(name) {
    ...
    for (const sourceFile of this.program.getSourceFiles()) {
      for (const statement of sourceFile.statements) {
        if (!ts.isModuleDeclaration(statement)
          || !ts.isStringLiteral(statement.name)
          || statement.name.text !== '@freddie/freddie-typert-protocol'
          || statement.body === undefined
          || !ts.isModuleBlock(statement.body)) continue
  ...
  isTypeMetaSymbol(node, name) {
    ...
    const registration = this.registrationForFile(declaration.getSourceFile().fileName)
    if (registration?.name === '@freddie/freddie-typert-protocol') return true
    for (let current = declaration; current !== undefined; current = optionalParent(current)) {
      if (ts.isModuleDeclaration(current)
        && ts.isStringLiteral(current.name)
        && current.name.text === '@freddie/freddie-typert-protocol') return true
    }
    return false
  }
...
function visibilityOf(node) {
  if ('name' in node && node.name !== undefined && ts.isPrivateIdentifier(node.name)) return 'private'
