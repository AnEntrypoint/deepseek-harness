...
dsh plugin --profile <name> add @freddie/freddie-subagent-claude-code
dsh plugin --profile <name> add @freddie/freddie-subagent-codex @freddie/freddie-subagent-claude-code
dsh plugin --profile <name> remove @freddie/freddie-subagent-codex
dsh plugin --profile <name> remove @freddie/freddie-subagent-claude-code
```
...
```sh
dsh plugin --profile tui add github:freddie/turtle-ui
...
Install external plugin bundles through `dsh plugin --profile <name> add <package-or-git-spec>`. The installed package owns its dependencies and contributes its declared `cordis.patch.yml` layer. The CLI also ships `@freddie/freddie-mcp-client` as a dependency for patch layers, but no MCP server is enabled by default because each server command is trusted executable code outside the agent sandbox.
